...
- **(macOS only)** Homebrew, for `libomp` dependency of LightGBM ([install guide](https://brew.sh))
...
**macOS only** — install `libomp` for LightGBM:
```bash
brew install libomp
```

**Linux / Windows** — no extra system packages needed. LightGBM's `pip` package bundles its own runtime.
...
- **Operator Dashboard** is protected — password: `ecoring2026` (configurable via the `ECORING_ADMIN_PW` environment variable)
...
2. Open `http://<that-ip>:8501` in your phone's browser
3. Use "Add to Home Screen" for a native-app feel (works in Safari on iOS, Chrome on Android)
...
> On Windows, use `python` instead of `python3` if the latter is not found.
...
**`venv` not found on Windows:**
Run: `python -m venv venv` (without `python3`)
...
Just press Enter to skip — email registration is optional.
